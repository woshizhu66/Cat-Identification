...
def predict_path(path, name2, count):
    box_list = []  # 创建坐标列表
    ...
    cv2.imwrite("D:/project/cat_sum/pic/{}/{}_{}.jpg".format(name2, name2, count), draw_1) # 将画过矩形框的图片保存到当前文件夹
    print(box_list)
    return box_list


for dirname, di, filenames in os.walk('D:/System/archive (1)/dataset/images'):
    for dirn in di:
        c = 0
        for dirnamse2, di2, filenames2 in os.walk(os.path.join(dirname, dirn)):
            for fName in filenames2:
                box_list = predict_path(os.path.join(dirnamse2, fName), os.path.basename(dirnamse2), c)
